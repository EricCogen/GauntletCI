...
- **GCI0036 labeler corrected**: `SilverLabelEngine` now detects getter mutations in addition to `[Pure]` attribute additions. Added `HasGetterMutationInDiff` helper that walks all diff lines (context + added) with the same getter-boundary state machine used by the rule. The previous heuristic only fired when `[Pure]` was added as a new line, causing every getter-mutation trigger to score as a false positive (labeler emitted negative label; rule fired positive finding). This was the root cause of GCI0036's 0% precision/0% recall. Confidence raised from 0.50 to 0.60 since getter-mutation detection is unambiguous when the getter boundary is visible in the diff hunk.
